...
class _WhiteSheetWidgetState extends State<WhiteSheetWidget> {
  ...
  Future<void> _fetchData() async {
    ...
    await Provider.of<PokemonAboutDataController>(context, listen: false).getPokemonAboutData(pokemon);
    if (!mounted) return;
    // fetch pokemon more info data
    await Provider.of<PokemonMoreInfoController>(context, listen: false).getPokemonMoreInfoData(pokemon);
    if (!mounted) return;
    // fetch pokemon stats data
    await Provider.of<PokemonStatsController>(context, listen: false).getPokemonStats(pokemon);
    if (mounted) {
      setState(() {
        loading = false;
      });
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius:
            const BorderRadius.only(topLeft: Radius.circular(constants.whiteSheetRadius), topRight: Radius.circular(constants.whiteSheetRadius)),
      ),
      child: Column(
        children: [
          SizedBox(
          ...
          SizedBox(height: screenHeight * .06, child: Center(child: customScrollerBuilder(isDark))),
          if (loading)
            const Expanded(
              child: Center(child: CircularProgressIndicator(color: constants.circularProgressIndicatorColor)),
            ),
          if (!loading)
            Expanded(
              child: PageView(
      ...
      child: Column(
        children: [
          Text(scrollTabs[tabIndex],
              style:
                  TextStyle(fontWeight: FontWeight.bold, color: _currentTabIndex == tabIndex ? (isDark ? Colors.white : Colors.black) : Colors.grey)),
  ...
  void updateTabIndex(int tabIndex) {
    setState(() {
      ...
      // add animation
      _tabController.animateToPage(tabIndex, duration: const Duration(milliseconds: 300), curve: Curves.linear);
    });
  }
}
