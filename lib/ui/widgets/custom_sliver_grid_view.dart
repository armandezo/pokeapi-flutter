...
class CustomSliverGridView extends StatelessWidget {
  ...
  const CustomSliverGridView({Key? key, this.showFavorites = false}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    final ThemeData themeData = Provider.of<ThemeController>(context).themeData;
    bool isDark = themeData == ThemeData.dark();
    return Consumer2<PokemonFavoritesController, PokemonBasicDataController>(builder: (_, favPokemonsProvider, allPokemonsProvider, ch) {
      ...
      return SliverGrid(
          delegate: SliverChildBuilderDelegate((context, index) {
            ...
            return PokemonCardItem(pokemon: pokemon, isDark: isDark, id: pokemonIdPadLeft, imageUrl: imageUrl);
