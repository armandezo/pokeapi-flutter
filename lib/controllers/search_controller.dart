...
class SearchPokemonsController with ChangeNotifier {
  ...
  Future<void> getAllPokemonsNames() async {
    final fetchedPokemonsAbilities = await pokemonSearchService.getAllAbilities();
    _pokemonsNames = fetchedPokemonsAbilities;
    notifyListeners();
  }

  Future<void> getAllAbilities() async {
    final fetchedPokemonsNames = await pokemonSearchService.getAllPokemonsNames();
  ...
  Future<Map<String, dynamic>> getPokemonDataByName(String name, bool isDarkTheme) async {
    final fetchedPokemonData = await pokemonBasicDataService.getPokemonByName(name);
