...
class PokemonsSearchService {
  Future<List<String>> getAllAbilities() async {
    ...
    try {
      final url = Uri.https('pokeapi.co', 'api/v2/pokemon', {'limit': '10'});
      final response = await http.get(url);
      if (response.statusCode == 200) {
        final fetchedPokemons = json.decode(response.body)['results'];
        for (var pokemonData in fetchedPokemons) {
          final String pokemonName = pokemonData['name'].substring(0, 1).toUpperCase() + pokemonData['name'].substring(1);
  ...
  Future<List<String>> getAllPokemonsNames() async {
    ...
    try {
      final url = Uri.https('pokeapi.co', 'api/v2/ability', {'limit': '10'});
      final response = await http.get(url);
      if (response.statusCode == 200) {
        final fetchedAbilities = json.decode(response.body)['results'];
        for (var pokemonData in fetchedAbilities) {
          final String ability = pokemonData['name'].substring(0, 1).toUpperCase() + pokemonData['name'].substring(1);
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for (var pokemon in fetchedPokemons) {
          pokemonsNames.add(pokemon['pokemon']['name'].substring(0, 1).toUpperCase() + pokemon['pokemon']['name'].substring(1));
        }
